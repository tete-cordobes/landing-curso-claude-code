...
async function deletePost(index) {
    ...
    // The sitemap and the static blog HTML are regenerated by the
    // build-blog workflow on every push to posts/. Nothing to do here.
    try {
        await deletePostFromGitHub(slug);
        showToast('✅ Post borrado en GitHub. El blog se regenera solo', 'success');
    } catch (error) {
        console.error('Error deleting post from GitHub:', error);
        showToast('⚠️ Post eliminado localmente, pero no se pudo borrar en GitHub', 'warning');
    }
}
        ...
        form.addEventListener('submit', async (e) => {
            ...
            if (saved) {
                ...
                if (found) {
                    ...
                    // Pushing the JSON is enough: the build-blog workflow
                    // regenerates the static HTML, the index and the sitemap.
                    try {
                        await savePostToGitHub(postData);
                        showToast('✅ Post guardado en GitHub. El blog se regenera solo en ~1 min', 'success');
                    } catch (error) {
...
// The sitemap generator that used to live here emitted URLs pointing at
// post-template.html and wiped every non-post entry. It is now owned by
// scripts/build-blog.mjs, which runs in CI and is the single writer of
// sitemap.xml, posts/index.json and blog/.
